...
const TriangleHight = ({title, triangleType}) => {
    ...
    const handleSubmit = (event) => {
      ...
      if (sideA && sideC) {
        setError(false);
        round
          ? setResult(Math.round(Math.sqrt((4 * (sideA * sideA) - (sideC * sideC))) / 2))
    ...
    return (
      <div className="flex items-center justify-center w-full mb-20 mt-20 flex-col">
        <h2 className="text-blue-600 text-3xl mb-7 mt-4">{title ? `Höhe ${title} berechnen`: "Höhe berechnen"}</h2>
        <p className="text-lg text-gray-500 font-bold mb-2">Gib die Längen der Seiten a und c an, um die Höhe zu berechnen</p>
